...
enum FilterType { goalsOnly, redCard, yellowCard, favoritePlayerShots }
...
class _MatchAnalysisScreenState extends State<MatchAnalysisScreen> {
  ...
  //  تحويل الفلتر إلى نص عربي للباكند
  String filterToArabic(FilterType type) {
    switch (type) {
      ...
      case FilterType.yellowCard:
        return "بطاقة صفراء";
      case FilterType.favoritePlayerShots:
        return "لقطات لاعب معين";
    }
  }
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              "Analysis finished: ${videoFilter.type}, Summary: ${videoFilter.summaryType}",
            ),
            backgroundColor: gradientStart,
          ),
        );
  ...
  Widget _buildFilterSelection() {
    return Column(
      ...
      children: [
        ...
        _buildFilterRadioTile("Goals Only", FilterType.goalsOnly),
        _buildFilterRadioTile("Red Card", FilterType.redCard),
        _buildFilterRadioTile("Yellow Card", FilterType.yellowCard),
        _buildFilterRadioTile(
          "Favorite player shots",
          FilterType.favoritePlayerShots,
        ),
      ],
    );
  }
  ...
  Widget _buildSummaryLengthSelection() {
    return Column(
      ...
      children: [
        ...
        _buildSummaryRadioTile("Long", SummaryLength.long),
        _buildSummaryRadioTile("Short", SummaryLength.short),
      ],
    );
  }
